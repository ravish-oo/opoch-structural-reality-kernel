...
-- Anyone can insert queries (both anonymous and authenticated users)
...
-- Drop existing policies on email_templates and emails_sent
DO $$ 
DECLARE
    pol record;
BEGIN
    -- Drop policies on email_templates
    FOR pol IN 
        SELECT policyname 
        FROM pg_policies 
...
-- Ensure anon role can insert into leads and queries
...
SELECT 
    ...
    'RLS is now properly configured. The apply form and query submissions should work without timeouts.' as message;